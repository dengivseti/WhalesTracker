...
export const OfferState: React.FC = ({children}) => {
    ...
    const saveOffer = async (offer: IOffer) => {
        ...
        try{
            let offers: IOffer[] = []
            const response = await request(`/api/edit/offers/edit`, 'POST', {...offer})
            if (response) {
    ...
    const deleteOffer = async (id: string) => {
        startLoading()
        try{
            const response = await request(`api/edit/offers/${id}`, 'DELETE')
            if (response) {
                const offers: IOffer[] = []
    ...
    return (
        <OfferContex.Provider value={{
            loading: state.loading,
            offers: state.offers,
            offer: state.offer,
            modal: state.modal,
            fetchOffer, saveOffer, setModal, deleteOffer, selectOffer,
        }}>
            {children}
