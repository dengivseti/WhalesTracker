...
export const GroupState: React.FC = ({ children }) => {
  ...
  const sortedStreams = (): void => {
    const sortedStreams = state.streams.sort((a: any, b: any) =>
  ...
  const removeStream = async (id: string) => {
    try {
      await request(
        `/api/edit/group/${state.group!._id}/${id}`,
        'DELETE',
      )
  ...
  const editFilter = (filter: IFilterValues) => {
    const filters = state.filters.filter(
      (f: IFilterValues) => f.position !== filter.position,
    )
    dispatch({
      type: 'EDIT_FILTER',
