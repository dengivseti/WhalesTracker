...
export const TableStatistic: React.FC = () => {
    ...
    if (!stats.length && !loading) {
        return (
            <Typography align={"center"} variant={'h5'}>NO DATA STATS</Typography>
        )
    }
    const name = type.toString()[0].toUpperCase() + type.toString().slice(1)
    const columns = [
        {
            name,
            options: {
    ...
    let sortValue = stats
    if (type !== 'day'){
        sortValue = stats.sort((a: any, b: any) => (a['hits'] as number) > (b['hits'] as number) ? -1 : 1)
    }
