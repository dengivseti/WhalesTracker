...
import {IStreamValues, typeIntervalDashboard, typeChartLineDashboard} from "../intrefaces/interface"
import {GroupContext} from "../context/GroupState"
import {Button, Checkbox, FormControl, FormControlLabel, Grid, InputLabel, Select} from "@material-ui/core"
import {makeStyles, Theme} from "@material-ui/core/styles"
import {intervalDashboard, linesChartDashboard} from "../utils/edit.utils"
...
export const MenuDashboard: React.FC = () => {
    ...
    const [interval, setInterval] = useState<typeIntervalDashboard>(menu.interval ? menu.interval : 'day')
    const [ignoreBot, setIgnoreBot] = useState<boolean>(menu.ignoreBot)
    const [typeLines, setTypeLines] = useState<typeChartLineDashboard>(menu.typeLines ? menu.typeLines : 'hits')
    ...
    const lineTypeHandler = (event: React.ChangeEvent<{value: unknown}>) => {
                ...
                <FormControl className={classes.checkboxControl}>
                    <FormControlLabel
                        ...
                        control={<Checkbox
                            color="primary"
                            checked={ignoreBot}
                            onChange={(event: React.ChangeEvent<HTMLInputElement>) => setIgnoreBot(event.target.checked)}
                        />}
                        label="Ignore bot"
                    />
                </FormControl>
