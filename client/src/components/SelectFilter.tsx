...
import {IFilterValues, typeFilter} from "../intrefaces/interface";
...
export const SelectFilter: React.FC = () => {
    ...
    const {filter, filters, addFilter, editFilter} = useContext(GroupContext)
    const [disabledBtn, setDisabledBtn] = useState<boolean>(true)
    const [filterChange, setFilterChange] = useState<typeFilter | null>(filter ? filter.name : null)
    const [filterValues, setFilterValues] = useState<string[] | boolean | null>(filter ? filter.action : null)
    ...
    const filterHandler = (event: React.MouseEvent) => {
        ...
        if (!filter){
            addFilter({name: filterChange!, action: filterValues!, position: filters.length + 1})
        }else{
            editFilter({name: filterChange!, action: filterValues!, position: filter.position})
        }
        setFilterChange(null)
        ...
        <>
            <Grid container direction="column" alignContent="center">
                <FormControl className={classes.form}>
                    ...
                    <Grid item>
                        <InputFieldFilter type={filterChange!} onValue={(item: string[] | boolean | null) => onValue(item)}/>
                    </Grid>
                    <Button
