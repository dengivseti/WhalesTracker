...
export const ModalStream: React.FC<IModalProps> = ({
  open,
  onClose,
  onSave,
}) => {
  ...
  const [relation, setRelation] = useState<boolean>(
    stream ? stream.relation : true,
  )
  const [label, setLabel] = useState<string>(
    stream ? stream.name : '',
  )
  const [typeRedirect, setTypeRedirect] = useState<typeRedirectValue>(
    stream ? stream.typeRedirect : 'httpRedirect',
  )
  const [formFormTypeRedirect, setFormFormTypeRedirect] = useState<
    IValueTypeRedirect
  >(
    stream
  ...
  const [isBot, setIsBot] = useState<boolean>(
    stream ? stream.isBot : false,
  )
  const [isActive, setIsActive] = useState<boolean>(
    stream ? stream.isActive : true,
  )
  const [useLog, setUseLog] = useState<boolean>(
    stream ? stream.useLog : true,
  )
  ...
  useEffect(() => {
    const form = listTypeRedirect.find(
      (type) => type.value === typeRedirect,
    )!
  ...
  return (
    <Dialog
      ...
      aria-labelledby="form-stream"
    >
      <DialogTitle id="form-stream">
        {stream ? stream.name : 'NEW STREAM'}
      </DialogTitle>
      <DialogContent>
        ...
        <Grid className={classes.formControl}>
          <FormControl className={classes.formControl}>
            <InputLabel htmlFor="typeRedirect">
              Type redirect
            </InputLabel>
            <Select
              native
              ...
              onChange={(
                ...
                setTypeRedirect(
                  event.target.value as typeRedirectValue,
                )
              }
              inputProps={{
                name: 'typeRedirect',
        ...
        <Grid className={classes.formControl}>
          <FieldCode
            type={formFormTypeRedirect.type}
        ...
        <Grid container justify="space-between">
          <FormLabel className={classes.checkboxControl}>
            Filtering values relation
          </FormLabel>
          <FormGroup row>
            <FormControlLabel
              control={
              ...
              <FormControlLabel
                ...
                control={
                  <Checkbox
                    ...
                    onChange={(
                      event: React.ChangeEvent<HTMLInputElement>,
                    ) => setUseLog(event.target.checked)}
                  />
                }
                label="Logging"
              />
      ...
      <DialogActions>
        <Button onClick={clickSaveHandler} color="primary">
          SAVE
        </Button>
      </DialogActions>
    </Dialog>
