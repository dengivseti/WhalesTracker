...
export const ModalOffer: React.FC<IModalProps> = ({
  open,
  onClose,
  onSave,
  offer,
  ...
  const [select, setSelect] = useState<typeDistributionOffer>(
    offer && offer.type ? offer.type : 'split',
  )
  const [isUsePercent, setIsUsePercent] = useState<boolean>(
    offer && offer.type
      ? listDistribution.find((d) => d.value === offer.type)!
          .isUsePercent
      : true,
  )
  const [fields, setFields] = useState<IOfferValue[]>(
    offer && offer.offers
      ? offer.offers
      : [
          {
  ...
  const selectHandler = (
    ...
    setSelect(value)
    setIsUsePercent(
      listDistribution.find((d) => d.value === value)!.isUsePercent,
    )
  }
