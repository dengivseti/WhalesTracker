...
export const ModalGroup: React.FC<IModalProps> = ({
  open,
  onClose,
}) => {
  ...
  const { error, clearStateError, addGroup, group } = useContext(
    GroupContext,
  )
          ...
          <Grid item xs={6} sm={6}>
            <FormControl>
              <TextField
                ...
                ) => setTimeUnic(Number(event.target.value))}
              />
            </FormControl>
          </Grid>
          <Grid item xs={6} sm={6}>
              ...
              <FormControlLabel
                control={
                  <Checkbox
                    ...
                    onChange={(
                      event: React.ChangeEvent<HTMLInputElement>,
                    ) => setCheckUniq(event.target.checked)}
                  />
                }
                label="Check unique"
              />
