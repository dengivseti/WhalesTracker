...
module.exports = class Filters {
    ...
    #isBrowser = (action) => action.includes(this.user.useragent.browser)
    ...
    #isNullUa = (action) => action ? !this.user.useragent.source : false
    ...
    #isBotSignature = (action) => {
        ...
        for (let i = 0; i < global.blackSignatures.length; i++) {
            if (this.user.useragent.source.includes(global.blackSignatures[i])){
                return true
            }
        }
        return false
    ...
    #isBlackIp = (action) => action ? !!ipRange(this.user.ip, global.blackIps) : false
